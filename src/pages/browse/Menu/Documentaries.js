...
const Documentaries = (props) => {
  ...
  useEffect(() => {
    const fetchMeals = async (type) => {
      ...
      setDocumentaries(responseData.results);
      detailCtx.data.push(responseData.results);
    };
    fetchMeals(requests.fetchDocumentaries);
  }, []);
  const btnDetail = (e) => {
  ...
  const dataList = documentaries.filter(
    (img) => srcURL === `https://image.tmdb.org/t/p/original${img.poster_path}`
  );

  return (
    <div className="container-fluid">
      <div className="text-white fw-bold h5 mt-3">Documentaries</div>
      <div
        ...
        <div className="carousel-inner">
          <div className="carousel-item active">
            <div className="d-flex justify-content-start">
              {documentaries.slice(0, 10).map((img) => (
                <div
          ...
          <div className="carousel-item">
            <div className="d-flex justify-content-start">
              {documentaries.slice(10, 20).map((img) => (
                <div
                  onClick={props.onClick}
                  key={img.id}
                  className={classes.bump}
