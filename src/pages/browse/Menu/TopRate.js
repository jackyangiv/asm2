...
const TopRate = (props) => {
  ...
  useEffect(() => {
    const fetchMeals = async (type) => {
      ...
      setTopRate(responseData.results);
      detailCtx.data.push(responseData.results);
    };
    fetchMeals(requests.fetchTopRated);
  }, []);
  ...
  const dataList = topRate.filter(
    (img) => srcURL === `https://image.tmdb.org/t/p/original${img.poster_path}`
  );

  return (
    <div className="container-fluid">
      <div className="text-white fw-bold h5 mt-3">Top Rate</div>
      <div
